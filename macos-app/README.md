# CodexSplit macOS 应用壳

这是 CodexSplit 的原生 macOS 外壳。它保留现有 Node 网关和 Dashboard，负责窗口、网关 sidecar 生命周期以及未来的 macOS 权限与菜单栏能力。
...
发布版会把 CodexSplit Voice Bar 内置到 `CodexSplit.app/Contents/Resources/OpenCodexBar.app`，主 App 会优先启动这个内置语音组件，用户不需要单独下载或安装语音伴侣。开发运行仍会探测常见外部路径，也支持 `OPENCODEX_BAR_PATH` 指向 `.app` 或源码构建目录。

打包时默认从仓库内的 `voice/OpenCodexBar` 读取或构建 CodexSplit Voice Bar，因此克隆 CodexSplit 后不再要求旁边另有一个旧语音仓库；发布构建也可以显式传入 `OPENCODEX_BAR_APP_PATH=/path/to/OpenCodexBar.app`，或者用 `OPENCODEX_BAR_SOURCE=/path/to/opencodex-bar` 覆盖源码路径。目录名和环境变量是旧版本兼容入口，不是对外产品名称。
...
Beta 产物为 `macos-app/build/CodexSplit-2.0.0-beta.1-arm64.dmg`。当前是本地 arm64 Ad-hoc 签名构建；正式分发前还需要 Apple Developer 签名和公证。
...
主窗口红色关闭按钮只关闭窗口，网关和内置语音组件继续运行；从 App 菜单或 Dock 选择“退出 CodexSplit”才会停止后台服务。
