...
test("release surfaces use package.json as the runtime version source", async () => {
  ...
  assert.equal(lockfile.version, "2.0.3");
  assert.equal(lockfile.packages[""].name, "codexsplit");
  assert.equal(lockfile.packages[""].version, "2.0.3");
  ...
  assert.match(plist, /<key>CFBundleShortVersionString<\/key>\s*<string>2\.0\.3<\/string>/);
  assert.match(plist, /<key>CFBundleVersion<\/key>\s*<string>2000030<\/string>/);
  assert.match(voicePlist, /<key>CFBundleIdentifier<\/key>\s*<string>com\.aitabby\.codexsplit\.voicebar<\/string>/);
  assert.match(voicePlist, /<key>CFBundleShortVersionString<\/key>\s*<string>2\.0\.3<\/string>/);
  assert.match(voicePlist, /<key>CFBundleVersion<\/key>\s*<string>2000030<\/string>/);
