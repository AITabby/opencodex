...
test("2.0.0-beta.1 release surfaces agree on the runtime version", async () => {
  ...
  assert.equal(lockfile.version, "2.0.0-beta.1");
  assert.equal(lockfile.packages[""].name, "codexsplit");
  assert.equal(lockfile.packages[""].version, "2.0.0-beta.1");
  ...
  assert.match(plist, /<key>CFBundleVersion<\/key>\s*<string>2000001<\/string>/);
  ...
  assert.match(voicePlist, /<key>CFBundleVersion<\/key>\s*<string>2000001<\/string>/);
  assert.match(packageScript, /MARKETING_VERSION=/);
  assert.match(packageScript, /BUNDLE_VERSION=/);
  assert.match(gateway, /name: "CodexSplit Engine V2", version: "2\.0\.0-beta\.1"/);
});
